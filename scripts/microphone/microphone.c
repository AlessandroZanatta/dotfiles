// gcc -Wall -Wextra -lasound -o microphone microphone.c
...
static int register_events_cb(snd_hctl_t *ctl, unsigned int mask,
...
int main() {
    ...
    if ((res = readlink("/proc/self/exe", command,
                        sizeof(command) - sizeof(script))) < 0) {
